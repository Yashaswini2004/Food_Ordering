  ...
  return (
    ...
    <img src={image} alt="" className='object-cover'/>
     ...
     <div className='flex justify-center items-center gap-2 text-green-500 text-lg font-semibold'>{type==="veg"?<TbMichelinStarGreen/>:<GiChickenOven/>}<span>{type}</span></div>
      ...
      <button className='w-full p-3 rounded-lg bg-green-300 text-white hover:bg-green-400 transition-all' onClick={()=>dispatch(AddItem({id:id,name:name,price:price,image:image,qty:1}))}>Add to dish</button>
    </div>
  )
}
